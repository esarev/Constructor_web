...
const getElement = (tagName, classNames, attributes) => {
	...
	if(attributes) {
		//перебираем (объект!)все свойства атрибутов
		for (const attribute in attributes) {
			//из объекта переносим св-ва в элемент
...
// создаём  блок методом деструктуризации:
const createMain = ({ title, main: { genre, rating, description, trailer, slider }}) => {
	...
	if (slider) {
		...
		const slides = slider.map(item => {
			...
			const swiperSlide = getElement('figcaption', ['card-description']);
